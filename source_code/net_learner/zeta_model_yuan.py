...
class NoisyLinear(nn.Linear):
    def __init__(self, in_features, out_features, sigma_init=0.095, bias=True):
    ...
    def forward(self, input):
        return F.linear(input, self.weight + self.sigma_weight * Variable(self.epsilon_weight).cuda(),
                        self.bias + self.sigma_bias * Variable(self.epsilon_bias).cuda())
...
class Net(nn.Module):
    def __init__(self, action_num=100, noise=0.095):
        ...
        self.fc = NoisyLinear(1280, 309, sigma_init=noise, bias=True)
        self.fc1 = NoisyLinear(1280, 309, sigma_init=noise, bias=True)
        self.device = 'cuda' if torch.cuda.is_available() else 'cpu'
    ...
    def loss_func(self, s, seq, a, value_target, bp):
        ...
        actor_loss_tmp = -torch.clamp(rho_action, max=2).detach() * log_pob * td_error.detach()
        rho_correction = torch.clamp(1 - 2 / rho_action, min=0.).detach()
...
class PNet(nn.Module):
    def __init__(self, action_num=100, noise=0.119):
        ...
        self.distribution = torch.distributions.Categorical
        self.lstm = nn.LSTM(180, 256, batch_first=True)
        self.lstm01 = nn.LSTM(180, 256, batch_first=True)

        self.fc = NoisyLinear(2304, 309, sigma_init=self.noise, bias=True)
        self.fc1 = NoisyLinear(2816, 309, sigma_init=0.0, bias=True)
    ...
    def forward(self, s, seq_input, s01=None):
        ...
        if s01 is None:
            ...
            s = s.view(s.size(0),1,16,15)
            s01 = s01.view(s01.size(0),1,24,15)
...
class KNet(nn.Module):
    def __init__(self, action_num=100, noise=0.119):
        ...
        self.distribution = torch.distributions.Categorical
        self.lstm = nn.LSTM(180, 256, batch_first=True)
        self.fc = NoisyLinear(2560, 28, sigma_init=0.085, bias=True)
        self.fc1 = NoisyLinear(2560, 28, sigma_init=0.0, bias=True)
        self.device = 'cuda' if torch.cuda.is_available() else 'cpu'

    def forward(self, s, seq):
        s = s.view(s.size(0), 1,20,15)
        seq, (h_n, _) = self.lstm(seq)
        seq = seq[:, -1, :]
        #print("s is ==",s.size())
        s = self.conv(s)
    ...
    def loss_func(self, s, seq, a, value_target, bp, alpha):
        ...
        rho_action = torch.gather(rho, 1, rho_a.long())
        rho_correction = torch.clamp(rho_action, max=2).detach() + torch.clamp(1 - 2 / rho_action, min=0.).detach()
