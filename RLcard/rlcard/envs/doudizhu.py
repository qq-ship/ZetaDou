...
class DoudizhuEnv(Env):
    ...
    def _extract_state(self, state):
        ...
        extracted_state['rule_actions'] = state['rule_actions']
        return extracted_state

    def _m_extract_state(self, state):
        ''' Encode state
        ...
        pp_id = state['landlord']
        if state['self'] == pp_id:  # landlord
            up = pp_id - 1
            down = pp_id + 1
            ...
            landlord_up_played_cards = _cards2array(state['played_cards'][up])
            landlord_down_played_cards = _cards2array(state['played_cards'][down])
            landlord_up_num_cards_left = _get_one_hot_array(state['num_cards_left'][up], 17)
            landlord_down_num_cards_left = _get_one_hot_array(state['num_cards_left'][down], 17)
            obs = np.concatenate((current_hand,
                                  others_hand,
                                  last_action,
                                  last_9_actions,
            ...
            # print("-=-=-=-==-=-=-=-=-::::",pp_id)
            landlord_played_cards = _cards2array(state['played_cards'][pp_id])
            ...
            last_landlord_action = _cards2array(last_landlord_action)
            landlord_num_cards_left = _get_one_hot_array(state['num_cards_left'][pp_id], 20)

            all_id = [0, 1, 2]
            all_id.remove(pp_id)
            all_id.remove(state['self'])
            # teammate_id = 3 - state['self']
            teammate_id = all_id[0]
            teammate_played_cards = _cards2array(state['played_cards'][teammate_id])
            last_teammate_action = 'pass'
            for i, action in reversed(state['trace']):
                if i == teammate_id:
