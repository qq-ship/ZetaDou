...
class Env(object):
    ...
    def m_reset(self):
        ...
        Returns:
            (tuple): Tuple containing:
                ...
                (int): The begining player
        '''
        self.game.rn = self.rn
        state, player_id = self.game.m_init_game()

        self.action_recorder = []

        self.pp_id = player_id
        self.pp_state = self._m_extract_state(state)

        return self._m_extract_state(state), player_id

    def m_player_id(self):
        return self.pp_id
    ...
    def m_step(self, action, raw_action=False):
        ''' Step forward
        ...
        return self._m_extract_state(next_state), player_id
    ...
    def m_run(self, is_training=False):

        trajectories = [[] for _ in range(self.num_players)]
        state, player_id = self.pp_state , self.pp_id
        ...
        while not self.is_over():
            ...
            # Environment steps
            next_state, next_player_id = self.m_step(action, self.agents[player_id].use_raw)
            # Save action
            trajectories[player_id].append(action)
    ...
    def get_state(self, player_id):
        ...
        '''
        return self._m_extract_state(self.game.get_state(player_id))
